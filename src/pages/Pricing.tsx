...
const modules = [
  {
    ...
    pillClassName: "bg-[#eef0ff] text-[#4f3dfc] border border-[#d9dcff]",
  ...
  },
  {
    ...
    pillClassName: "bg-[#e9fbf1] text-emerald-600 border border-[#ccefdc]",
...
function renderCell(cell: CapabilityCell) {
  ...
  if (cell.type === "text") {
    return (
      <span className="text-[13px] font-semibold text-[#4f3dfc]">{cell.value}</span>
    );
  }
  return (
...
function FeatureList({ plan }: { plan: Plan }) {
  return (
    <div className={`mt-8 border-t pt-8 ${plan.dark ? "border-white/10" : "border-slate-100"}`}>
      <p className={`text-[13px] font-extrabold tracking-[0.18em] uppercase ${plan.dark ? "text-[#8791cb]" : "text-[#4f3dfc]"}`}>
        {plan.key === "standard"
      ...
      <ul className="mt-5 space-y-4">
        {plan.features.map((feature) => (
          <li key={feature.label} className="flex items-start gap-3">
            <span className={`mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${plan.dark ? "bg-white/10 text-white" : "bg-emerald-50 text-emerald-500"}`}>
              <Check className="h-3.5 w-3.5" />
            </span>
            <span className={`text-[15px] leading-6 ${plan.dark ? "text-white" : "text-slate-700"}`}>
              {feature.label}
            </span>
          </li>
        ))}
...
function PlanCard({ plan, onTalkToSales }: { plan: Plan; onTalkToSales: () => void }) {
  return (
    <motion.article
      ...
      className={`relative overflow-hidden rounded-[30px] p-8 md:p-10 ${plan.cardClassName} ${plan.dark ? "" : "bg-white"}`}
    >
      {plan.featured && (
      ...
      <div className={`flex items-start gap-4 ${plan.featured ? "pr-[120px]" : ""}`}>
        ...
        <div className="pt-1">
          ...
          <p className={`mt-2 text-[13px] font-extrabold tracking-[0.18em] uppercase ${plan.dark ? "text-[#8fa0ff]" : "text-[#4f3dfc]"}`}>
            Lucid Suite
          </p>
        </div>
      </div>
      <p className={`mt-8 text-[16px] leading-7 ${plan.dark ? "text-slate-300" : "text-slate-600"}`}>
        {plan.description}
      </p>
...
/* ─────────────────────────────────────────────────────────────────────────
   Shared page-container — mirrors the nav's exact centering + padding so
   every section's left edge lines up with the logo at every breakpoint.
   nav uses:  max-w-[1180px] mx-auto w-full px-4 sm:px-6
   We match:  max-w-[1180px] mx-auto w-full px-4 sm:px-6
───────────────────────────────────────────────────────────────────────── */
function PageContainer({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`mx-auto w-full max-w-[1180px] px-4 sm:px-6 ${className}`}>
      {children}
    </div>
  );
}
...
export default function Pricing() {
  ...
  const onTalkToSales = () => navigate("/contact");

  return (
    <div className="min-h-screen bg-white text-slate-900">

      {/* ── Hero ─────────────────────────────────────────────────────── */}
      <section className="pb-20 pt-28 lg:pt-32">
        ...
        <PageContainer>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-2">
            {modules.map((module) => (
              ...
              >
                <span className={`inline-flex rounded-full px-4 py-2 text-[14px] font-bold ${module.pillClassName}`}>
                  {module.pill}
                </span>
      ...
      <section className="pb-28">
        <PageContainer>
          ...
          <div className="mt-10 overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_16px_34px_rgba(15,23,42,0.08)]">
            <div className="overflow-x-auto">
              <table className="min-w-[600px] w-full border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/70">
                    <th className="w-[36%] px-4 sm:px-6 py-8 text-left text-[15px] font-bold text-slate-500">
                      Key Platform Features
                    </th>
                    <th className="px-3 sm:px-6 py-8 text-center">
                      <div className="text-[20px] sm:text-[28px] font-extrabold leading-none text-[#4f3dfc]">Standard</div>
                      <div className="mt-2 hidden sm:block text-[13px] text-slate-500">Fundamental assessments</div>
                      <button type="button" onClick={onTalkToSales} className="mt-2 text-[12px] sm:text-[13px] font-bold text-[#4f3dfc] hover:underline">
                        Talk to Sales
                      </button>
                    </th>
                    <th className="relative bg-[#f3efff] px-3 sm:px-6 py-8 text-center">
                      <div className="text-[20px] sm:text-[28px] font-extrabold leading-none text-slate-900">Advanced</div>
                      <div className="mt-2 hidden sm:block text-[13px] text-slate-500">+ Conversational Agent</div>
                      <button type="button" onClick={onTalkToSales} className="mt-2 text-[12px] sm:text-[13px] font-bold text-[#4f3dfc] hover:underline">
                        Talk to Sales
                      </button>
                    </th>
                    <th className="px-3 sm:px-6 py-8 text-center">
                      <div className="text-[20px] sm:text-[28px] font-extrabold leading-none text-slate-900">Pro</div>
                      <div className="mt-2 hidden sm:block text-[13px] text-slate-500">+ Sales Acceleration</div>
                      <button type="button" onClick={onTalkToSales} className="mt-2 text-[12px] sm:text-[13px] font-bold text-[#4f3dfc] hover:underline">
                        Talk to Sales
                      </button>
                    </th>
                  ...
                  {capabilityGroups.map((group) => (
                    <React.Fragment key={group.title}>
                      <tr className="border-b border-slate-200 bg-white/95">
                        <td colSpan={4} className="px-4 sm:px-6 py-4 text-[11px] sm:text-[13px] font-black tracking-[0.18em] text-[#4f3dfc]">
                          {group.title}
                        </td>
                      </tr>
                      {group.rows.map((row) => (
                        <tr key={row.label} className="border-b border-slate-200 last:border-b-0">
                          ...
                          <td className="px-3 sm:px-6 py-5 text-center">{renderCell(row.standard)}</td>
                          <td className="bg-[#f3efff] px-3 sm:px-6 py-5 text-center">{renderCell(row.advanced)}</td>
                          <td className="px-3 sm:px-6 py-5 text-center">{renderCell(row.pro)}</td>
                        </tr>
                      ))}
                    </React.Fragment>
                  ))}
        ...
        <PageContainer>
          ...
          <div className="mt-12">
            <Accordion className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_14px_32px_rgba(15,23,42,0.08)]">
              {faqs.map((faq) => (
                <AccordionItem key={faq.question} value={faq.question} className="border-b border-slate-200 last:border-b-0">
                  <AccordionTrigger className="px-6 sm:px-8 py-7 text-left text-[17px] sm:text-[20px] font-bold text-slate-900 hover:no-underline">
                    <span className="pr-6">{faq.question}</span>
                  </AccordionTrigger>
                  <AccordionContent className="px-6 sm:px-8 pb-7 pt-0 text-[16px] leading-8 text-slate-600">
                    {faq.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
      ...
      <section className="pb-24">
        <PageContainer>
          <div className="rounded-[30px] bg-slate-950 px-6 sm:px-8 py-12 text-center text-white shadow-[0_18px_40px_rgba(15,23,42,0.2)]">
            <p className="text-[13px] font-black tracking-[0.24em] text-[#8fa0ff]">READY TO TALK?</p>
