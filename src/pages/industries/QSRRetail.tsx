...
const PROBLEM_FIX_ROWS = [
  {
    ...
    fixCopy:
      "Launch brief deployed to every associate's phone before go-live. Readiness score by outlet — before the launch, not after the complaint.",
  },
  {
    ...
    fixCopy:
      "Execution score by outlet, updated every week. Revenue correlation visible. Best practice identified and replicated to underperforming locations.",
  },
  {
    ...
    fixCopy:
      "Root cause mapped to the knowledge gap behind the deviation. Fix deployed to the right team. Comprehension verified. Recurrence tracked.",
  },
  {
    ...
    fixCopy:
      "Mobile photo-capture audit against your VM standard. Non-compliance flagged and corrective action assigned the same day — not at the next site visit.",
  },
  {
    ...
    fixCopy:
      "Role-specific capability deployed from Day 1 on WhatsApp. Capability score by associate from Week 1. Manager alerted when score drops — not when the resignation lands.",
  },
];
...
function FlipCard({ row }: { row: ProblemFixRow }) {
  return (
  ...
  return (
    <div className="lucid-qsr">
      <style>{`
        ...
        .metric-preview-grid + .metric-preview-grid {
          margin-top: 12px;
        }
        ...
        <div className="hero-grid">
          <div className="reveal" data-reveal>
            ...
            <h1 className="hero-title text-4xl md:text-6xl font-extrabold leading-tight">
              Revenue made or<br />
              lost<br />
              in <span>30 seconds</span><br />
              at the counter.
            </h1>
            <p className="hero-copy text-lg text-slate-600 mt-6">
              Your floor team either knows what to do — or they don't.{" "}
              <span className="font-semibold text-slate-900">
                Right now, most organizations have no way to know which side they're on.
              </span>
            </p>
...
>
  Request a pilot
          ...
          <div className="dashboard-preview reveal" data-reveal>
            <div className="dash-card">
              ...
              <div style={{ padding: 16 }}>
                {[METRICS.slice(0, 2), METRICS.slice(2)].map((group, index) => (
                  <div key={index} className="metric-preview-grid">
                    {group.map((metric) => (
                      <div key={metric.label} className="metric-preview">
                        <div className="metric-preview-label">{metric.label}</div>
                        <div className="metric-preview-value" style={{ color: metric.color }}>{metric.value}</div>
                        <div className="metric-preview-sub">{metric.sub}</div>
                      </div>
                    ))}
                  </div>
                ))}
        ...
        <div className="reveal" data-reveal>
          ...
          <div className="flip-grid">
            {PROBLEM_FIX_ROWS.slice(0, 3).map((row) => (
              <FlipCard key={row.problemTitle} row={row} />
            ))}
          </div>
      ...
      <SectionWrapper id="qsr-cta" maxWidth={1100}>
        <div className="cta reveal" data-reveal>
          <div>
            <h3 className="text-3xl md:text-5xl font-extrabold leading-tight text-white">
              See your outlet #47.<br />
              <span style={{ color: "var(--accent)" }}>Know what to fix by tomorrow.</span>
            </h3>
